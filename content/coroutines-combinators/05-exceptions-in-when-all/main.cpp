...
// Same when_all, with the error path spelled out: if any task failed, its stored
// exception is kept and rethrown from await_resume, so a failure in one of the
// parallel tasks surfaces at the caller's co_await as an ordinary exception.
struct WhenAll {
    ...
    std::vector<int> await_resume() {
        if (error) std::rethrow_exception(error);   // first failure wins
        return std::move(results);
    }
};

Task<int> ok(int id, long ms) {
    co_await Sleep{ms};
    co_return id * 10;
}

Task<int> failing(long ms) {
    co_await Sleep{ms};
...
Task<int> program() {
    std::vector<Task<int>> tasks;
    tasks.push_back(ok(1, 10));
    tasks.push_back(failing(20));
    tasks.push_back(ok(3, 30));
    try {
        std::vector<int> r = co_await WhenAll{std::move(tasks)};
        std::cout << "got " << r.size() << " results (not reached)\n";
    } catch (const std::exception& e) {
        std::cout << "when_all surfaced a failure: " << e.what()
                  << " (at t=" << g_loop.now << ")\n";
    }
    co_return 0;
}
