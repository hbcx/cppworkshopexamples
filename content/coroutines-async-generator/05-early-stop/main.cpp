...
// Could produce five values, but only ever advances when pulled.
async_generator<int> upTo5() {
    for (int i = 1; i <= 5; ++i) {
        co_await Sleep{10};
        ++g_produced;
        co_yield i;
...
Task takeTwo(async_generator<int>& gen) {
    int taken = 0;
    while (co_await gen.next()) {
        std::cout << "  took " << gen.value() << "\n";
        if (++taken == 2)
            break;                 // stop early; we never pull the rest
    }
    co_return;
}

int main() {
    async_generator<int> gen = upTo5();
    Task c = takeTwo(gen);
    c.start();
    g_loop.run();
    std::cout << "producer actually produced " << g_produced << " of a possible 5\n";
    return 0;
}
