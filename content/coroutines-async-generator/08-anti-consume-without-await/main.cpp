...
// ANTI-PATTERN: treating an async stream like an ordinary range.
//
//   for (int v : stream()) { ... }        // does not compile: advancing is a
//                                          // co_await, so there is no plain
//                                          // begin()/end() with a ++ iterator
//
// An async stream has no synchronous iterator, because getting the next element
// may require suspending. There is no way to "just read" the next value without
// a coroutine to co_await in -- trying to force it would mean blocking the thread
// until the element arrives, which defeats the whole point.

// FIX: consume it from inside a coroutine with a co_await loop.
