...
// The producer awaits 20 units of "work" before each element.
...
// An independent task ticking every 15 units, to show the loop stays free while
// the stream is awaiting -- the two interleave on one thread.
Task ticker() {
    for (int k = 0; k < 3; ++k) {
        co_await Sleep{15};
...
Task consume(async_generator<int>& gen) {
    while (co_await gen.next())
        std::cout << "  element " << gen.value() << "@t" << g_loop.now << "\n";
    co_return;
}
...
int main() {
    ...
    Task t = ticker();
    Task c = consume(gen);
    t.start();       // both are in flight while the stream awaits its work
    c.start();
    g_loop.run();
    return 0;
}
